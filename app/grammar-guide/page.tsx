...
export default function GrammarGuideDashboard() {
    ...
    useEffect(() => {
        const loadCompletedActivities = async () => {
            ...
            try {
                const completed = await apiService.getCompletedActivities(token);
            ...
            <div className="max-w-4xl mx-auto px-4 py-8 md:px-8">
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-[2rem] p-8 md:p-12 mb-10 text-white shadow-lg">
                    ...
                    <p className="text-indigo-100 font-medium text-lg max-w-xl">
                        120 interactive units based on the visual learning method. Master
                        tenses, rules, and avoid common mistakes.
                    </p>
                </div>
              ...
              `}
                ...
                <div className="space-y-4 pb-12">
                    {filteredUnits.map((unit) => {
                        const isLocked = unit.exercises && unit.exercises.length === 0;
                        const isCompleted = completedUnits.includes(String(unit.id));
                        ...
                        return (
                            <button
                                ...
                                className={`
                  w-full flex items-center p-5 rounded-3xl border-2 text-left transition-all relative
                  ${
                                    isLocked
                ...
                `}
                            >
                                {isCompleted && !isLocked && (
                                    <div className="absolute top-4 right-4 flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-black shadow-sm">
                                        <CheckCircle size={14} />
                                        Hecha
                                    </div>
                                )}
                    ...
                    ${
                                        isLocked
                                            ...
                                            : isCompleted
                                                ...
                                                : "bg-indigo-100 text-indigo-600 group-hover:scale-110 transition-transform"
                                    }
                  `}
                                >
                                    {isLocked ? <Lock size={24} /> : isCompleted ? <CheckCircle size={24} /> : <Book size={24} />}
                                </div>
