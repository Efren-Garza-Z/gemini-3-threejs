...
export default function ChatPage() {
    ...
    useEffect(() => {
        ...
        // Small delay just to let the client settle and prevent flashes
        ...
        <main className="h-screen w-screen bg-gradient-to-br from-blue-100 via-teal-50 to-green-100 overflow-hidden flex flex-col pt-24">
            ...
            {!isChecking && !token && (
                <div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
                    <div className="bg-white/80 backdrop-blur-xl p-10 rounded-[40px] border border-white max-w-md shadow-2xl">
                        <div className="bg-orange-100 text-orange-600 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 transform rotate-3">
                            <span className="text-4xl">🔐</span>
                        </div>
                        <h2 className="text-3xl font-black text-zinc-900 mb-4 tracking-tight">Acceso Exclusivo</h2>
                        <p className="text-zinc-600 font-medium mb-8 leading-relaxed">
                            Para interactuar con el Agente de IA y guardar tu historial personalizado de conversaciones, es necesario iniciar sesión.
                        </p>
                        <button 
                            onClick={() => window.location.href = "/auth"} 
                            className="w-full bg-teal-600 text-white font-bold text-lg py-4 rounded-2xl hover:bg-teal-700 hover:scale-105 transition-all shadow-lg"
                        >
            ...
            {!isChecking && token && (
                <div className="flex flex-col md:flex-row flex-1 pb-6 px-4 md:px-12 gap-6 overflow-hidden animate-in fade-in duration-1000">
                                ...
                                history.map((msg) => (
                                    <div key={msg.id} className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                        ...
                                        <div className="flex justify-end mb-4">
                                            <div className="bg-orange-200 text-orange-950 px-5 py-3 rounded-2xl rounded-tr-none max-w-[85%] shadow-lg text-sm font-medium">
                                                {msg.prompt}
                                            </div>
                                        </div>
                ...
                <textarea
                    ...
                    className="flex-1 bg-transparent p-3 outline-none text-zinc-800 resize-none max-h-32 custom-scrollbar font-medium placeholder-zinc-400"
                    style={{ height: 'auto' }}
                    onInput={(e: any) => {
                ...
                />
                                <button
                                    onClick={handleSend}
                                    className="bg-orange-200 text-orange-950 p-3 rounded-xl hover:bg-orange-300 transition-colors"
                                >
                                    <ArrowUp size={20} />
                                </button>
                            </div>
